...
$channel->queue_bind(
    ...
    CONF_RABBIT_ROUTING_KEY_NAME_CREATE_REFUND
);
 ...
 *
 */
$RefundFaker = new MakeFaker();
for ($cont = 1; $cont <= 1; $cont++) {
    $msgCreate = new AMQPMessage(
        $RefundFaker->RefundFaker(),
        array('delivery_mode' => AMQPMessage::DELIVERY_MODE_PERSISTENT)
    );

    $channel->basic_publish($msgCreate, CONF_RABBIT_NAME, CONF_RABBIT_ROUTING_KEY_NAME_CREATE_REFUND);
